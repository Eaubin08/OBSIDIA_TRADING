...
Implémentation de référence du domaine Trading gouverné par Obsidia — **v0.2**.
...
## Current status — Reference Implementation v0.2
...
Voir `docs/FREEZE_MANIFEST_V0.2.md` pour le manifest complet, et `merkle_seal.json` (+ `docs/SEAL_SCOPE.md`) pour le périmètre scellé. La v0.1 (F1→F10, avant la démo Naive vs Governed) reste une référence historique intacte et indépendamment vérifiable sous `docs/history/` — voir `docs/DEMO_BRANCH_SEAL_STATUS.md` pour le détail de la promotion.
